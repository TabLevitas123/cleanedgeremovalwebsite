...
const WhyChooseUsSection = () => {
    ...
    <section className="py-16 md:py-24 bg-neutral-50 dark:bg-neutral-800">
      <div className="container mx-auto px-4">
        ...
        {/* Differentiator Grid */}
        <framer_motion_1.motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-12 mb-16" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
          {differentiators.map((item) => (<framer_motion_1.motion.div key={item.id} className="flex flex-col items-center text-center" variants={fadeIn}>
              <div className={`w-16 h-16 rounded-full bg-${item.color}/10 dark:bg-${item.color}/20 flex items-center justify-center mb-4`}>
                <Icon_1.default name={item.icon} size="large" className={`text-${item.color}`}/>
              </div>
        ...
        <div className="mt-16 pt-12 border-t border-neutral-200 dark:border-neutral-700 bg-primary-color text-white rounded-lg py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {stats.map((stat) => (<framer_motion_1.motion.div key={stat.id} variants={fadeIn}>
...
exports.default = WhyChooseUsSection;
//# sourceMappingURL=WhyChooseUsSection.jsx.map