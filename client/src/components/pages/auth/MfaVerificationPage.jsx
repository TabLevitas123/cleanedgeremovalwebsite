...
const MfaVerificationPage = () => {
    ...
    const [resendTimer, setResendTimer] = (0, react_1.useState)(0);
    const [isSubmitted, setIsSubmitted] = (0, react_1.useState)(false);
    // Error state
    const [error, setError] = (0, react_1.useState)('');
    // API mutation hooks
    const [verifyMfa, { isLoading: isVerifying }] = (0, authSlice_1.useVerifyMfaMutation)();
    const [resendMfaCode, { isLoading: isResending }] = (0, authSlice_1.useResendMfaCodeMutation)();
    ...
    return (<div>
      <div className="text-center mb-8">
        ...
        <p className="text-neutral-600 dark:text-neutral-400 mt-2">
          {email ? (<>Enter the 6-digit code sent to <strong>{email}</strong></>) : (<>Enter the 6-digit code sent to your email</>)}
        </p>
      </div>
      
      {error && (<div className="bg-error-color bg-opacity-10 text-error-color p-4 rounded-md mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Code Input */}
        <div>
          <label htmlFor="code-0" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-3">
            Verification Code
          </label>
          <div className="flex justify-center gap-2">
            {code.map((digit, index) => (<input key={index} id={`code-${index}`} ref={(el) => (inputRefs.current[index] = el)} type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6} value={digit} onChange={(e) => handleInputChange(index, e.target.value)} onKeyDown={(e) => handleKeyDown(index, e)} className={`w-12 h-14 text-center text-xl font-semibold border rounded-md focus:outline-none focus:ring-2 focus:ring-primary-color dark:bg-neutral-800 dark:text-neutral-200 dark:border-neutral-600 ${error ? 'border-error-color' : 'border-neutral-300'}`} disabled={isVerifying} aria-label={`Digit ${index + 1} of verification code`}/>))}
          </div>
          <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400 text-center">
            Didn't receive a code?{' '}
            <button type="button" onClick={handleResendCode} disabled={resendTimer > 0 || isResending || !email} className={`text-primary-color hover:text-primary-light focus:outline-none ${(resendTimer > 0 || isResending || !email) ? 'opacity-50 cursor-not-allowed' : ''}`}>
        ...
        {/* Submit Button */}
        <Button_1.default type="submit" variant="primary" size="large" fullWidth disabled={isVerifying || code.some(digit => !digit)} icon={isVerifying ? 'loader' : undefined} className="mt-6">
...
exports.default = MfaVerificationPage;
//# sourceMappingURL=MfaVerificationPage.jsx.map